...
function renderFieldLinkGroups(title, fields) {
  const items = Array.isArray(fields)
    ? fields.filter((field) => Array.isArray(field?.urls) && field.urls.length)
    : [];
  if (!items.length) return '';
  return `<div class="vendor-inline-list"><strong>${esc(title)}</strong><div>${items.map((field) => `<div><strong>${esc(field.key || 'Reference')}</strong>: ${field.urls.map((url) => `<a href="${esc(url)}" target="_blank" rel="noreferrer">${esc(url)}</a>`).join(', ')}</div>`).join('')}</div></div>`;
}
